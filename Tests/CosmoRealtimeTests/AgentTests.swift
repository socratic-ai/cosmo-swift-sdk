...
struct AgentTests {
    ...
    @Test func mergesMcpToolsIntoConfig() async throws {
        ...
        let agent = try Agent(tools: [caller], mcp: McpRegistry(servers: [McpStdioServer(name: "fs", command: "x")]))
    ...
    @Test func noMcpPreservesNilTools() async throws {
        let agent = try Agent(tools: [], mcp: nil)
    ...
    @Test func agentForwardsHooksToConfig() async throws {
        ...
        let agent = try Agent(hooks: hooks)
        let capturedBox = CaptureBox<SessionConfig>()
        let agentSession = try await agent.start(
    ...
    @Test func agentHooksAppendAfterConfigHooks() async throws {
        ...
        let agent = try Agent(hooks: agentHooks)
        let capturedBox = CaptureBox<SessionConfig>()
        let agentSession = try await agent.start(
